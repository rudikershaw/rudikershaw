...
METR paid senior engineers to work on real high impact open source project tasks. The projects selected, were selected because they had extremely high pull request merging standards and exceptionally low rates of bugs being introduced. This meant the research automatically inherited a quality bar. The tasks weren't _proxies_ for real work (like pull requests or HTTP servers). The developers were asked to estimate the work in the pool, then given a task, and then randomly assigned to using or not-using AI. Their screen was recorded for the duration of their work (and therefore timed). These developers worked on 246 real issues, averaging two hours of work on each issue. After the research was complete, the developers were surveyed to find out how much faster they thought they were working using AI coding assistants.
...
A lofty goal. So how are they proposing we do this? Asking an LLM to do it is an option, but that would be costly, un-auditable, non-deterministic, and risks introducing all the biases that LLMs are established to have (self-preference bias in particular^13). If you're a machine learning aficionado, and I say "[Reinforcement Learning with Human Feedback](https://en.wikipedia.org/wiki/Reinforcement_learning_from_human_feedback)" (RLHF), then perhaps an idea occurs to you. 

A _Reward Model_ is a model trained to predict human preferences on output of some kind. In RLHF real human reviews produce scores of, for example, LLM output. The LLM output and the scores, are then used to train the reward model to predict these scores on future output. This reward model can then be used to aid in the training of the original LLM. The folks at SWEPR trained something like a reward model to mimic expert human reviewer's scores for a variety of code quality and value measures. This model can then be run on arbitrary code to answer questions like "is this just a refactor with no improvements?", "is this code loosely coupled?", "is this new code a maintenance burden?", "how quickly could I implement this?", "how experienced was the engineer who produced this code?". They published a paper on this model in 2024^12. This paper is also a nice accessible read.
...
12. Y Denisov-Blanch, I Ciobanu, S Obstbaum, M Kosinski. Predicting Expert Evaluations in Software Code Reviews (2024). arXiv 2409.15152 (2024)

13. Koki Wataoka, Tsubasa Takahashi, Ryokan Ri. Self-Preference Bias in LLM-as-a-Judge (2024). https://arxiv.org/abs/2410.21819